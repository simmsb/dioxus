...
impl ClientRequest {
    ...
    fn fetch_inner(method: http::Method, path: String, query: String) -> ClientRequest {
        ...
        let mut server_url = get_server_url().to_owned();

        if server_url.is_empty() {
            server_url = "http://this.is.not.a.real.url:9000".to_owned();
        } else if server_url.starts_with('/') {
            server_url = format!("http://this.is.not.a.real.url:9000{}", server_url.trim_end_matches('/'));
        }
